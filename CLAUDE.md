...
- Library `libcode` `"115"` is the WYLD-wide library; `circmap.py` uses it to generate the "Complete Circ Map Policy" page showing all CMAP records unfiltered. `policy_cards.py`'s `WYLD_LIBCODE` constant is the same value, used to gate which policy cards/subnav links appear per library (e.g. the Recirculating/Library-Use profile reports added by `wyldprofiles.py` are WYLD-only).

**Field indexing gotcha:**
`SCHEMAS` indexes directly into `line.split('|')`, where `fields[0]` is the record-type token (e.g. `"UPRF"`) itself. If you're checking field positions with `cut -d'|' -f N` against the raw policies file, `cut` counts that record-type token as field 1 — so a schema index is always `cut` field number minus 1. Verify any new field mapping against `mock-data/policies` for a record whose value you can predict before trusting the offset (e.g. UPRF: `LIBRARYUSE` is recirculating and library-use, `17PA` is a patron profile and isn't; LOCN: `STACKS` is available/holdable/not-shadowed, `UNKNOWN` is not-available/not-holdable/shadowed). The original `LOCN` schema had `available`/`holdable` swapped — introduced without this verification — before being corrected.
...
`wyldprofiles.py` also generates four reference policy reports the same way: `itemtype/wyld.html` (flat table from `records['ITYP']`), `location/wyld.html` (flat table from `records['LOCN']`, with extra Available/Holdable/Shadowed Yes-No columns), and `itemcategory/wyld.html`/`usercategory/wyld.html` (one table per `ICT1`-`ICT10`/`CAT1`-`CAT10`, sections with no records silently skipped). The `ICT`/`CAT` `SCHEMAS` entries share the layout `{1: 'code', 2: 'name', 3: 'description'}`.

**Shared rendering:**
- `html_utils.py` — `page()` wraps a body string in the full HTML shell with header/nav/styles; `table()` renders a list of rows into a Bootstrap table; `lib_nav(lib, libcode, current_dir, lookups)` builds the per-page subnav strip.
- `policy_cards.py` — single source of truth for which policy pages exist per library, alphabetized by title. Both `libindex.py` (hub page cards) and `html_utils.lib_nav` (subnav buttons) derive from `available_cards(lib, libcode, lookups)` so the two can't drift out of sync. Add new policy pages here, not by editing `libindex.py` or `html_utils.py` directly.
